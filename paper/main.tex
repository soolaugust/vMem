...
\title{\textbf{vMem: Systematic OS Abstractions for LLM Context\\with Verifiable Pin Guarantees}}
...
\begin{abstract}
Persistent memory for LLM agents has adopted eviction policies from OS memory management, but existing systems map at most one or two kernel primitives---leaving critical guarantees unaddressed. We present \textbf{vMem}, an agent memory layer that systematically maps seven OS memory subsystems (demand paging, kswapd, DAMON, mlock, CRIU, kworker, shared memory) to agent-memory operations. Our key technical contribution is \emph{pin semantics}: hard/soft memory locking that provably guarantees constraint survival under arbitrary eviction pressure---a property no existing system provides.

We evaluate on the LongMemEval benchmark (500 questions across 6 question types), achieving 92.6\% retrieval session recall with sub-millisecond latency (0.09ms p50) and zero external dependencies. Under 4$\times$ memory pressure, pinned constraints achieve 100\% physical survival (82\% lexically retrievable), while systems without pins retain 0\%. Quantitative ablation on LongMemEval confirms each OS primitive contributes a distinct, irreplaceable guarantee: removing any single primitive causes binary loss of a system property.

The system is deployed as an open-source Claude Code plugin (MIT) with 10{,}700+ test assertions, demonstrating that production-grade agent memory requires only a single SQLite file and principled application of OS abstractions. Code and benchmarks: \url{https://github.com/soolaugust/vMem}.
\end{abstract}
...
\begin{tikzpicture}[
  ...
  event/.style={fill=blue!8, draw=blue!50, rounded corners=2pt, inner sep=2.5pt, font=\tiny},
  bad/.style={fill=red!12, draw=red!70, rounded corners=2pt, inner sep=2.5pt, font=\tiny},
  good/.style={fill=green!12, draw=green!70, rounded corners=2pt, inner sep=2.5pt, font=\tiny},
]

% === Top: Without Pin Semantics ===
\node[anchor=west, font=\footnotesize\bfseries] at (0, 3.2) {(a) Without pin semantics (existing systems)};
\draw[timeline] (0,2.5) -- (9.5,2.5) node[right, font=\tiny] {time};

\node[event] (e1) at (1.0, 2.9) {Store constraint};
\node[event] (e2) at (3.2, 2.9) {Memory pressure};
\node[bad]   (e3) at (5.8, 2.9) {\textbf{Constraint evicted}};
\node[bad]   (e4) at (8.5, 2.9) {\ding{55} Agent violates};

\draw[->, red!80, thick] (e2.east) -- (e3.west);
\draw[->, red!80, thick] (e3.east) -- (e4.west);

% Chunk bar
\fill[blue!25] (0.2, 2.0) rectangle (5.0, 2.2);
\fill[red!25] (5.0, 2.0) rectangle (9.2, 2.2);
\node[font=\tiny] at (2.5, 1.85) {constraint alive};
\node[font=\tiny] at (7.0, 1.85) {constraint gone};

% === Bottom: With Pin Semantics ===
\node[anchor=west, font=\footnotesize\bfseries] at (0, 1.1) {(b) With pin semantics (vMem)};
\draw[timeline] (0,0.4) -- (9.5,0.4) node[right, font=\tiny] {time};

\node[event] (p1) at (1.0, 0.8) {Store + \textbf{pin}};
\node[event] (p2) at (3.2, 0.8) {Memory pressure};
\node[good]  (p3) at (5.8, 0.8) {Pin blocks eviction};
\node[good]  (p4) at (8.5, 0.8) {\ding{51} Agent complies};

\draw[->, green!60!black, thick] (p2.east) -- (p3.west);
\draw[->, green!60!black, thick] (p3.east) -- (p4.west);

% Chunk bar (solid throughout)
\fill[green!25] (0.2, -0.1) rectangle (9.2, 0.1);
\node[font=\tiny] at (4.7, -0.25) {constraint alive (pinned, immune to eviction)};

\end{tikzpicture}%
}% end resizebox
\caption{The pin-guarantee problem. (a)~Existing systems evict constraints under memory pressure, causing agents to re-violate established rules. (b)~vMem's \texttt{mlock}-inspired pin semantics guarantee constraint survival regardless of pressure.}
\label{fig:motivation}
\end{figure}

Recent systems address this by providing persistent memory stores for agents, building on Retrieval-Augmented Generation (RAG)~\cite{rag2020lewis}. Mem0~\cite{mem0}, Letta (MemGPT)~\cite{memgpt2023}, Zep~\cite{zep2024}, and A-Mem~\cite{amem2025} offer vector-based retrieval with various augmentations. More recently, AMV-L~\cite{amvl2026} treats agent memory as a ``managed systems resource'' with lifecycle-driven eviction, and HTM-EAR~\cite{htmear2026} employs hierarchical tiered memory with importance-aware eviction watermarks.
...
Table~\ref{tab:coverage} summarizes which OS primitives each system implements. No prior system covers more than two; vMem covers all seven.
...
\midrule
\textbf{vMem} & \checkmark & \checkmark & \checkmark & \checkmark & \checkmark & \checkmark & \checkmark \\
\bottomrule
\end{tabular}
\end{table}
...
\end{tikzpicture}
\caption{vMem architecture as a data-flow diagram. Chunks flow through write (blue), retrieval (green), eviction (red), and checkpoint (purple) paths. Pin state (orange) blocks eviction; WAL mode enables concurrent multi-agent access (yellow). OS primitive analogs annotated.}
\label{fig:arch}
\end{figure*}
...
\begin{table*}[t]
\centering
\caption{Complete OS $\to$ vMem primitive mapping.}
\label{tab:mapping}
\footnotesize
\begin{tabular}{lp{4.2cm}p{4.8cm}p{3cm}}
\toprule
\textbf{OS Primitive} & \textbf{OS Semantics} & \textbf{vMem Mapping} & \textbf{Impl.} \\
\midrule
...
Traditional agent memory systems pre-load a fixed top-$K$ context at session start (analogous to loading all possible pages at process creation). vMem instead implements \emph{demand paging}: the agent's reasoning is the ``process,'' and a knowledge gap triggers a ``page fault''---a call to \texttt{memory\_lookup(query)}.
...
\paragraph{Automatic pin decisions.} A key design question is \emph{what should be pinned}. Our deployed system uses a rule-based policy: chunks are auto-pinned when (1) \texttt{chunk\_type} $\in$ \{\texttt{design\_constraint}, \texttt{api\_contract}\}, (2) importance $\geq$ 0.9, or (3) the user explicitly marks them via \texttt{pin\_memory()}. This policy achieves high precision (pinned items are indeed critical) but unknown recall (some constraints may not be typed correctly). Future work could learn pin decisions from agent behavior: a constraint that, when evicted, causes the agent to produce corrected output is a candidate for retroactive pinning. This connects pin semantics to reinforcement learning from human feedback---the pin becomes a learned invariant rather than a manual annotation.

\subsection{CRIU: Session Checkpoint/Restore}

At session end (Stop hook), vMem snapshots the current working set---recently accessed chunks, active project context, and session metadata---into a checkpoint file. At next SessionStart, this checkpoint is used to ``warm start'' the session, restoring the agent to approximately its prior cognitive state without requiring full re-retrieval.
...
vMem is implemented in Python (3.12+) as a single-file SQLite database with an MCP (Model Context Protocol) server interface. Key implementation details:

\paragraph{Storage.} All state resides in a single SQLite~\cite{sqlite2020hipp} database file (WAL mode). The schema uses FTS5 virtual tables for full-text search (BM25 scoring~\cite{robertson2009bm25}), with standard tables for chunk metadata, pin state, and access tracking.
...
\paragraph{Hook Integration.} vMem ships as a Claude Code plugin with lifecycle hooks at six events: SessionStart, UserPromptSubmit, PreToolUse, PostToolUse, Stop, and PreCompact.
...
We evaluate vMem on four dimensions: (1) retrieval quality on a standard benchmark, (2) constraint survival under pressure, (3) multi-agent coherence, and (4) quantitative ablation. All experiments are reproducible via included scripts.
...
\paragraph{Analysis.} vMem achieves 92.6\% session recall using only BM25+FTS5 (no embedding model). Performance is strongest on single-session queries (97--100\%) and weakest on temporal reasoning (90.3\%), where paraphrased time references challenge lexical matching. Figure~\ref{fig:lme_results} visualizes the per-type breakdown.

\paragraph{Cross-system comparison.} Table~\ref{tab:comparison} contextualizes our results against publicly reported numbers. vMem achieves competitive retrieval recall without embeddings, while uniquely providing pin guarantees and multi-agent sharing.

\begin{table}[t]
\centering
\caption{Cross-system comparison on LongMemEval. $\dagger$Reported in Mem0 evaluation (April 2026). vMem is the only system with pin + multi-agent guarantees.}
\label{tab:comparison}
\footnotesize
\begin{tabular}{lccc}
\toprule
System & LME Score & Embed? & Pin? \\
\midrule
Mem0 v3$^\dagger$      & 94.8 & Yes & No \\
Full-context$^\dagger$ & 72.1 & N/A & No \\
RAG baseline$^\dagger$ & 67.0 & Yes & No \\
\midrule
\textbf{vMem}   & \textbf{92.6} & No & \textbf{Yes} \\
\bottomrule
\end{tabular}
\end{table}
...
\paragraph{Latency advantage.} At 0.09ms p50 per query (vs.\ 35ms for embedding-based search), vMem enables demand-paging semantics where the agent retrieves \emph{during reasoning} without perceptible delay---making multi-round retrieval practical within a single LLM turn.
...
\midrule
vMem (BM25)           & 0.22 $\pm$ 0.01 & 0.21 $\pm$ 0.01 \\
Vector (embedding only)    & \textbf{0.90} $\pm$ 0.02 & \textbf{0.73} $\pm$ 0.03 \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Analysis.} Embedding search substantially outperforms BM25 on vocabulary-shifted queries (0.90 vs.\ 0.22). This is expected: BM25 requires lexical overlap while embeddings capture semantic similarity. We include this benchmark to honestly characterize the retrieval trade-off.

\paragraph{Hybrid retrieval.} vMem's OS framework is backend-agnostic---the pin, eviction, and checkpoint semantics operate on chunk metadata regardless of how retrieval scoring works. As a proof-of-concept, we implemented Reciprocal Rank Fusion (RRF)~\cite{rrf2009cormack} combining BM25 with nomic-embed-text embeddings, achieving Recall@5 = 0.75 (vs.\ 0.22 BM25-only, 0.80 embedding-only). The hybrid approach retains all OS guarantees while closing the retrieval gap. We keep BM25-only as the default for zero-dependency deployment; hybrid mode is an optional sysctl-gated enhancement.

\paragraph{Production context.} In the LongMemEval benchmark where queries use contextual terms (as agents do during reasoning), BM25 achieves 92.6\% recall---demonstrating that the paraphrasing gap is an artificial worst-case, not representative of real agent usage patterns.
...
\textbf{vMem}    & \textbf{100\%}   & \textbf{82\%}    & Yes \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Analysis.} Under 4$\times$ pressure, systems without pin semantics retain 0\% of constraints---eviction has no mechanism to distinguish critical from expendable items. vMem guarantees 100\% physical survival (all 50 pinned chunks remain in the database regardless of pressure). The 82\% retrievability gap reflects BM25 limitations on paraphrased verification queries, not a pin failure.

\paragraph{End-to-end constraint violation.} To measure downstream impact, we simulate a multi-session scenario: 20 realistic design constraints are stored, 200 work items accumulate, then eviction reduces the store to 50 chunks. An agent then queries for each constraint using natural-language paraphrases. Without pins, \textbf{100\%} of constraints are unretrievable (all evicted). With pins, the violation rate drops to \textbf{30\%} (constraints survive physically; the 30\% gap is purely lexical mismatch). Pin semantics reduce constraint violations by 70\% --- the remaining gap is addressable through hybrid retrieval (\S\ref{sec:paraphrase}).
...
\paragraph{Comparison.} Systems using separate per-agent stores (the default for mem0, Zep) achieve 0\% cross-agent visibility without explicit synchronization protocols. vMem's shmem-via-WAL approach eliminates this class of coordination problems.

\subsection{Scalability and Latency}

\begin{table}[t]
\centering
\caption{Search latency at 10{,}000 chunks (measured, 1000 queries, 5 seeds). vMem requires no external model; vector baseline uses nomic-embed-text (274MB).}
\label{tab:latency}
\small
\begin{tabular}{lcc}
\toprule
...
\midrule
\textbf{vMem (FTS5)} & \textbf{0.09} & \textbf{0.15} \\
...
We ablate each OS primitive on the LongMemEval retrieval benchmark and constraint survival test. Each variant removes one primitive while keeping others intact (Table~\ref{tab:ablation}).

\begin{table*}[t]
\centering
\caption{Quantitative ablation: each row removes one primitive. LME Recall measured on LongMemEval (500 questions, per-question isolation). Survival measured under 4$\times$ pressure.}
\label{tab:ablation}
\small
\begin{tabular}{lcccl}
\toprule
Configuration & LME Recall & $\Delta$ & Constraint Survival & Property Lost \\
\midrule
Full system              & 0.926 & ---    & 100\% & (none) \\
$-$ pin (mlock)          & 0.926 & 0.0\%  & 0\%   & Constraint survival guarantee \\
$-$ demand paging        & 0.625 & $-$32.5\% & 100\% & Context-specific retrieval \\
$-$ eviction (kswapd)    & 0.926 & 0.0\%  & 100\% & Bounded memory growth \\
$-$ WAL sharing (shmem)  & 0.926 & 0.0\%  & 100\% & Multi-agent visibility \\
\bottomrule
\end{tabular}
\end{table*}
...
\begin{enumerate}[leftmargin=*,nosep]
  \item \textbf{Pin removal} causes binary loss of constraint survival (100\%$\to$0\%) with zero impact on retrieval quality---confirming pin semantics as an orthogonal guarantee layer.
  \item \textbf{Demand paging removal} (pre-loading top-$K$ at session start using the most recent session as query, instead of per-question retrieval) reduces recall by 32.5\% (0.926$\to$0.625). Context-specific queries outperform even well-chosen static preloads because the relevant session cannot be anticipated before the question is asked.
  \item \textbf{Eviction and WAL removal} do not affect retrieval quality but eliminate bounded growth and multi-agent sharing respectively---properties that become critical at scale but are not captured by recall metrics.
\end{enumerate}

Each primitive contributes exactly one irreplaceable property. Notably, these are \emph{qualitatively different} losses: pin removal is binary (100\%$\to$0\%), demand paging removal is graceful ($-$32.5\%), and eviction/WAL losses are invisible to retrieval metrics entirely.

%% ============================================================
\section{Deployment Experience}
\label{sec:deployment}

vMem has been deployed as a Claude Code plugin by the first author in daily kernel development and software engineering workflows. We report qualitative observations from single-user deployment (not a controlled user study):
...
\paragraph{Cross-store federation (VFS layer).} Multiple vMem instances (personal, team, organization) could be unified through a virtual filesystem layer, analogous to Linux's VFS that presents ext4, NFS, and tmpfs through a single interface.
...
We presented vMem, an agent memory system that systematically maps seven OS memory-management primitives to agent-memory operations. Our key contributions are: (1) a complete OS$\to$agent taxonomy identifying which guarantee each primitive enables; (2) verifiable pin semantics that provably guarantee constraint survival under arbitrary pressure; and (3) empirical validation on LongMemEval (500 questions, 92.6\% retrieval recall) with quantitative ablation confirming each primitive is necessary.
