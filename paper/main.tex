...
\title{\textbf{0CompactMem: Systematic OS Abstractions for LLM Agent Memory\\with Verifiable Pin Guarantees}}

\author{
  Zhidao Su \\
  Xiaomi Inc. \\
  \texttt{suzhidao@xiaomi.com}
}
...
\begin{abstract}
Persistent memory for LLM agents has adopted eviction policies from OS memory management, but existing systems map at most one or two kernel primitives---leaving critical guarantees unaddressed. We present \textbf{0CompactMem}, an agent memory layer that systematically maps seven OS memory subsystems (demand paging, kswapd, DAMON, mlock, CRIU, kworker, shared memory) to agent-memory operations. Our key technical contribution is \emph{pin semantics}: hard/soft memory locking that provably guarantees constraint survival under arbitrary eviction pressure---a property no existing system provides.
...
The system is deployed as an open-source Claude Code plugin (MIT) with 10{,}700+ test assertions, demonstrating that production-grade agent memory requires only a single SQLite file and principled application of OS abstractions. Code and benchmarks: \url{https://github.com/soolaugust/0CompactMem}.
\end{abstract}
...
\node[anchor=west, font=\footnotesize\bfseries] at (0, 1.1) {(b) With pin semantics (0CompactMem)};
...
\caption{The pin-guarantee problem. (a)~Existing systems evict constraints under memory pressure, causing agents to re-violate established rules. (b)~0CompactMem's \texttt{mlock}-inspired pin semantics guarantee constraint survival regardless of pressure.}
...
\end{tikzpicture}
\caption{0CompactMem architecture as a data-flow diagram. Chunks flow through write (blue), retrieval (green), eviction (red), and checkpoint (purple) paths. Pin state (orange) blocks eviction; WAL mode enables concurrent multi-agent access (yellow). OS primitive analogs annotated.}
\label{fig:arch}
\end{figure*}
...
\paragraph{Analysis.} 0CompactMem achieves 92.6\% session recall using only BM25+FTS5 (no embedding model). Performance is strongest on single-session queries (97--100\%) and weakest on temporal reasoning (90.3\%), where paraphrased time references challenge lexical matching. Figure~\ref{fig:lme_results} visualizes the per-type breakdown.

\paragraph{Cross-system comparison.} Table~\ref{tab:comparison} contextualizes our results against publicly reported numbers. 0CompactMem achieves competitive retrieval recall without embeddings, while uniquely providing pin guarantees and multi-agent sharing.

\begin{table}[t]
\centering
\caption{Cross-system comparison on LongMemEval. $\dagger$Reported in Mem0 evaluation (April 2026). 0CompactMem is the only system with pin + multi-agent guarantees.}
...
\midrule
\textbf{0CompactMem}   & \textbf{92.6} & No & \textbf{Yes} \\
\bottomrule
\end{tabular}
\end{table}
...
\paragraph{Latency advantage.} At 0.09ms p50 per query (vs.\ 35ms for embedding-based search), 0CompactMem enables demand-paging semantics where the agent retrieves \emph{during reasoning} without perceptible delay---making multi-round retrieval practical within a single LLM turn.
...
\midrule
0CompactMem (BM25)           & 0.22 $\pm$ 0.01 & 0.21 $\pm$ 0.01 \\
...
\textbf{0CompactMem}    & \textbf{100\%}   & \textbf{82\%}    & Yes \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Analysis.} Under 4$\times$ pressure, systems without pin semantics retain 0\% of constraints---eviction has no mechanism to distinguish critical from expendable items. 0CompactMem guarantees 100\% physical survival (all 50 pinned chunks remain in the database regardless of pressure). The 82\% retrievability gap reflects BM25 limitations on paraphrased verification queries, not a pin failure.
...
\paragraph{Comparison.} Systems using separate per-agent stores (the default for mem0, Zep) achieve 0\% cross-agent visibility without explicit synchronization protocols. 0CompactMem's shmem-via-WAL approach eliminates this class of coordination problems.

\subsection{Scalability and Latency}

\begin{table}[t]
\centering
\caption{Search latency at 10{,}000 chunks (measured, 1000 queries, 5 seeds). 0CompactMem requires no external model; vector baseline uses nomic-embed-text (274MB).}
\label{tab:latency}
\small
\begin{tabular}{lcc}
\toprule
