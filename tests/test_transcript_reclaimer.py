...
def test_transcript_reclaimer_preserves_tail_appended_in_replace_window(tmp_path: Path, monkeypatch) -> None:
    ...
    appended = _line({"type": "user", "message": {"role": "user", "content": "late-start " + "q" * 50_000 + " late-end"}})
    ...
    assert "late-start" in backup
    assert "late-end" in backup
...
def test_transcript_reclaimer_bounds_oversized_concurrent_tail(tmp_path: Path, monkeypatch) -> None:
    ...
    appended = _line({"type": "user", "message": {"role": "user", "content": "tail-start " + "y" * 50_000 + " tail-end"}})
    original_write_bytes = Path.write_bytes
...
def test_transcript_reclaimer_fallback_uses_original_line_indices(tmp_path: Path) -> None:
    ...
    reclaim_transcript(transcript, memory_dir=memory_dir, target_bytes=2_500, keep_tail_lines=8, max_line_bytes=100)
